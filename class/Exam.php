...
class Exam
{
	...
	private $examStatusTable = 'online_user_exam_status';
	private $conn;
	...
	public function enrollToExam()
	{
		if ($this->exam_id) {
			...
			$sqlStatus = "
				INSERT INTO " . $this->examStatusTable . " (`user_id`, `exam_id`, `status`)
				VALUES (?, ?, 'Start')";
			$this->conn->begin_transaction();

			try {
				$stmtEnroll = $this->conn->prepare($sqlEnroll);
				$stmtEnroll->bind_param("ii", $_SESSION["userid"], $this->exam_id);
				$stmtEnroll->execute();

				$stmtStatus = $this->conn->prepare($sqlStatus);
				$stmtStatus->bind_param("ii", $_SESSION["userid"], $this->exam_id);
				$stmtStatus->execute();
	...
	public function get_exam_stime($examid)
	{
		if ($examid) {
			$sqlQuery = "SELECT start_time FROM online_exam_process WHERE examid = ? AND userid = ?";
			$stmt = $this->conn->prepare($sqlQuery);
			if ($stmt === false) {
				throw new Exception('SQL prepare error: ' . $this->conn->error);
			}
			$stmt->bind_param('ii', $examid, $_SESSION["userid"]);
			$stmt->execute();
			$result = $stmt->get_result();
			$start_time = null;
	...
	public function getExamStatus()
	{
		if ($_SESSION["userid"] && $this->exam_id) {
			$sqlQuery = "SELECT status FROM online_user_exam_status WHERE exam_id = ? AND user_id = ?";
			$stmt = $this->conn->prepare($sqlQuery);
			if ($stmt === false) {
				throw new Exception('SQL prepare error: ' . $this->conn->error);
			}
			$stmt->bind_param('ii', $this->exam_id, $_SESSION["userid"]);
			$stmt->execute();
			$result = $stmt->get_result();
			$status = '';
			if ($row = $result->fetch_assoc()) {
	...
	public function getExamSubmited()
	{
		if ($_SESSION["userid"] && $this->exam_id) {
			$sqlQuery = "SELECT submit FROM online_user_exam_status WHERE exam_id = ? AND user_id = ?";
			$stmt = $this->conn->prepare($sqlQuery);
			if ($stmt === false) {
				throw new Exception('SQL prepare error: ' . $this->conn->error);
			}
	...
	{
		$sqlQuery = "
			SELECT exam.id, exam.exam_title, exam.duration, exam.total_question, exam.marks_per_right_answer, exam.marks_per_wrong_answer, answer.marks, status.submit, status.status AS examStatus
            ...
            LEFT JOIN " . $this->examStatusTable . " AS status ON exam.id = status.exam_id AND status.user_id = enroll.user_id
		...
		while ($exam = $result->fetch_assoc()) {
			...
			$examDateTime = $this->get_exam_stime($exam['id']);
			$duration = $exam['duration'] . ' minute';
			$examEndTime = strtotime($examDateTime . '+' . $duration);
			$examEndTime = date('Y-m-d H:i:s', $examEndTime);
			$currentTime = date("Y-m-d") . ' ' . date("H:i:s", strtotime(date('h:i:sa')));
			$examSubmited = $exam['submit'];


			if ($exam['examStatus'] == 'Start' && $currentTime < $examDateTime) {
				$displayStatus = 'Start';
				$examMessage = "Start Exam";
			} else if ($examDateTime && $currentTime >= $examDateTime && $currentTime <= $examEndTime && $examSubmited !='Submited') {
				$displayStatus = 'Pending';
				$examMessage = "Finish Exam";
				$this->updateExamStatus($exam['id'], $displayStatus);
			} else if ($examDateTime && $currentTime > $examEndTime || $examSubmited =='Submited') {
				$displayStatus = 'Completed';
				$examMessage = "View Result";
				$this->updateExamStatus($exam['id'], $displayStatus);
			}

			$buttonLabel = empty($examMessage) ? 'Start Exam' : $examMessage;
			$colStatus = empty($examMessage) ? 'Start' : $displayStatus;
			...
			// $rows[] = $exam['examStatus'];
			$rows[] = $colStatus;
			$rows[] = '<a type="button" name="view" href="process_exam.php?exam_id=' . $exam["id"] . '" class="btn btn-info btn-xs">' . $buttonLabel . '</a>';
			$records[] = $rows;
		}
	...
	public function getUserExam($userid) {
		$sqlQuery = "
			SELECT 
				exam.id, 
				exam.exam_title, 
				exam.duration, 
				exam.total_question, 
				exam.marks_per_right_answer, 
				exam.marks_per_wrong_answer, 
				answer.marks, 
				status.submit, 
		...
		$stmtTotal = $this->conn->prepare("SELECT * FROM " . $this->examTable . " WHERE id = '" . $userid . "'");
		$stmtTotal->execute();
		$allResult = $stmtTotal->get_result();
		$allRecords = $allResult->num_rows;
		...
		while ($exam = $result->fetch_assoc()) {
			...
			$rows[] = '-' . $exam['marks_per_wrong_answer'] . ' Mark';
			$rows[] = $exam['submit'];
			$rows[] = '<a type="button" name="view" href="user_exam_result.php?exam_id=' . $exam["id"] . '" class="btn btn-info btn-xs">' . $exam['examStatus'] . '</a>';
			$records[] = $rows;
		}
		...
		error_log(json_encode($output)); // Log the output for debugging
		echo json_encode($output);
	}
	...
	{
		$sqlQuery = "UPDATE " . $this->examStatusTable . " SET status = ? WHERE exam_id = ? AND user_id = ?";
		$stmt = $this->conn->prepare($sqlQuery);
		$stmt->bind_param("sii", $status, $examId, $_SESSION["userid"]);
		$stmt->execute();
	}
	...
	{
		$sqlQuery = "UPDATE " . $this->examStatusTable . " SET submit = ? WHERE exam_id = ? AND user_id = ?";
		$stmt = $this->conn->prepare($sqlQuery);
		$status = 'Submited';
		$userId = $_SESSION["userid"];
		$stmt->bind_param("sii", $status, $examId, $userId);
	...
	public function loadQuestions()
	{
		if ($this->exam_id) {
			...
			while ($questions = $result->fetch_assoc()) {
				...
				$decodedQuestion = strip_tags($questions["question"], $allowed_tags);
				...
				while ($options = $resultQuestion->fetch_assoc()) {
					...
					<div class="col-md-6" style="margin-bottom:32px;">
						<div class="radio">
							<label><h5><input type="radio" name="option_1" class="answer_option" data-question_id="' . $questions["question_id"] . '" data-id="' . $count . '"/>&nbsp;' . $count . ')&nbsp;&nbsp;' . $options["title"] . '</h5></label>
						</div>
					</div>';
	...
	public function updateExamAnswer()
	{
		if ($this->exam_id && $this->question_id && $this->answer_option) {
			...
			$marks = 0;
			$addmark = $examDetails['marks_per_right_answer'];
			$submark = $examDetails['marks_per_wrong_answer'];
			...
			$stmt->bind_param("ssiii", $this->answer_option, $marks, $_SESSION["userid"], $this->exam_id, $this->question_id);
	...
	{
		if ($questionId) {
			$sqlQuery = "SELECT id, question_id, option, title FROM online_exam_option WHERE question_id = '" . $questionId . "'";
			$stmt = $this->conn->prepare($sqlQuery);
			$stmt->execute();
			$result = $stmt->get_result();
			$records = array();
	...
	public function examCompleted()
	{
		if ($_SESSION["userid"] && $this->exam_id) {
			$sqlQuery = "
				SELECT *
				FROM " . $this->examStatusTable . " 			
				WHERE status='Completed' AND user_id = ? AND exam_id = ?";
			$stmt = $this->conn->prepare($sqlQuery);
