...
class Questions
{
	...
	public function listQuestions()
	{
		...
		while ($question = $result->fetch_assoc()) {
			$rows = array();
			$rows[] = $question['id'];
			$allowed_tags = '<h1><h2><h3><h4><h5><h6><br><ol><ul><li><small><s><em><strong><p><u><b><i>';
			$decodedQuestion = strip_tags($question["question"], $allowed_tags);
			$decodedQuestion = str_replace(array('\r\n'), '<br>', $decodedQuestion);
			$decodedQuestion = preg_replace('/ {2,}/', '&nbsp;&nbsp;', $decodedQuestion);
			$rows[] = $decodedQuestion;
	...
	public function getQuestion()
	{
		if ($this->question_id) {
			...
			while ($question = $result->fetch_assoc()) {
				...
				$allowed_tags = '<h1><h2><h3><h4><h5><h6><br><ol><ul><li><s><small><em><strong><p><u><b><i>';
				$decodedQuestion = strip_tags($question["question"], $allowed_tags);
				$decodedQuestion = str_replace(array('\r\n'), '<br>', $decodedQuestion);
				$decodedQuestion = preg_replace('/ {2,}/', '&nbsp;&nbsp;', $decodedQuestion);
				$rows['question'] = $decodedQuestion;
	...
	public function insert()
	{
		...
		if ($this->exam_id && $this->question_title && $this->answer_option) {
			...
			$this->question_title = $this->conn->real_escape_string($this->question_title);
